...
const Demographics = ({
		...
		return Array.from(Object.keys(data.demographic.year)).map((year) => (
			<li key={year}>
				{year +
					...
					' (' +
					Math.round(
						(data.demographic.year[year] / data.submitted) * 100,
					) +
					'%)'}
			</li>
		));
	...
	const parseDomains = (
		...
		return Array.from(Object.keys(data.demographic.schools)).map(
			(school) => (
				<li key={school}>
					{school +
						': ' +
					...
					<ul className={styles.statusList}>
						...
						<li>
							<Typography className={styles.entry}>
								...
								/>{' '}
								confirmed:{' '}
								{data.demographic.schools[school].confirmed}
							</Typography>
						</li>
						<li>
							<Typography className={styles.entry}>
								...
								/>{' '}
								admitted:{' '}
								{data.demographic.schools[school].admitted}
							</Typography>
						</li>
					</ul>
				</li>
		...
		<div className={styles.section}>
			...
			<div className={styles.subsection}>
				<Typography className={styles.entry}>
					...
					{Math.round(
						((data.demographic.gender['Female'] || 0) /
							data.submitted) *
							100,
					)}
					%)
				...
				<Typography className={styles.entry}>
					...
					{Math.round(
						((data.demographic.gender['Male'] || 0) /
							data.submitted) *
							100,
					)}
					%)
				...
				<Typography className={styles.entry}>
					...
					{Math.round(
						((data.demographic.gender['Other'] || 0) /
							data.submitted) *
							100,
					)}
					%)
