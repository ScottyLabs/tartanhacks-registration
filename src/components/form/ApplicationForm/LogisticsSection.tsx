...
const LogisticsSection = ({
	...
	const [attendingPhysically, setAttendingPhysically] =
	...
	const validateForm = async () => {
		...
		if (!isValidPhoneNumber(phoneNumber)) {
			...
			setPhoneNumberHelper(
				'The phone number format you entered is invalid.',
			);
		} else {
	...
	useEffect(() => {
		if (fetchedProfile) {
			...
			setAttendingPhysically(
				logisticsFields.attendingPhysically ?? false,
			);
	...
	return (
      ...
      /> */}
			...
			<Autocomplete
				...
				value={shirtSize}
				onChange={(e, value) =>
					setShirtSize(value?.split(' ')[0] as ShirtSize)
				}
				renderInput={(params) => (
					<TextField
						variant="outlined"
